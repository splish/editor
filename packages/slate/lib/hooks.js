...
var createInitialState = exports.createInitialState = function createInitialState() {
  return {
    editorState: _slate.Raw.deserialize({
      nodes: [{
        kind: 'block',
        type: _paragraph.P,
        nodes: [{
...
var unserialize = exports.unserialize = function unserialize(_ref) {
  ...
  if (serialized) {
    return { editorState: _slate.Raw.deserialize(serialized, options) };
...
var serialize = exports.serialize = function serialize(_ref2) {
  ...
  return {
    serialized: _slate.Raw.serialize(editorState, options)
  };
};
...
var merge = exports.merge = function merge(states) {
  ...
  var mergedEditorState = _slate.State.create({ document: mergedDocument });
...
var split = exports.split = function split(state) {
  ...
  return nodes.map(function (node) {
    ...
    var splittedEditorState = _slate.State.create({ document: splittedDocument });
...
var handleRemoveHotKey = exports.handleRemoveHotKey = function handleRemoveHotKey(_, _ref3) {
  ...
  return new Promise(function (resolve, reject) {
    return _slate.Plain.serialize(editorState).length < 1 ? resolve() : reject();
  });
};
